/* 该小工具会将当前被封禁的用户用删除线划去。
 * 取自[[zhwp:MediaWiki:Gadget-markblocked.js]]，oldid=67626938。
 * 已萌百化
 * Fixed with ♥ by User:Leranjun
 */
(function($, mw) {
	function markBlocked(container) {
		...
		setCSS('\
			.user-blocked-temp {' + (window.mbTempStyle || 'opacity: 0.7; text-decoration: line-through!important') + '}\
			.user-blocked-indef {' + (window.mbIndefStyle || 'opacity: 0.4; font-style: italic; text-decoration: line-through!important') + '}\
			.user-blocked-partial {' + (window.mbPartialStyle || 'text-decoration: underline!important; text-decoration-style: dotted!important') + '}\
		...
		var ignoredUsers = ["示例用户", "Example"].concat(window.mbIgnoredUsers || []);
		// Find all "user" links and save them in userLinks : { 'users': [<link1>, <link2>, ...], 'user2': [<link3>, <link3>, ...], ... }
		contentLinks.each(function(i, lnk) {
			if ($(lnk).hasClass('mw-changeslist-date') || $(lnk).parent('span').hasClass('mw-history-undo') || $(lnk).parent('span').hasClass('mw-rollback-link')|| $(lnk).parent('span').hasClass('comment')) return;
			url = $(lnk).attr('href');
			if (!url)  return;
			...
			const localurl = url.replace(window.location.origin, '');
			if (ma = scriptRX.exec(localurl)) {
				pgTitle = ma[1];
			} else if (ma = articleRX.exec(localurl)) {
				pgTitle = ma[1];
			} else {
				return;
			}
		...
		var serverTime,
			apiRequests = 0;
		var waitingCSS = setCSS('a.user-link {opacity:' + (window.mbLoadingOpacity || 0.85) + '}', 'markBlockedStyle-2', 'add');
